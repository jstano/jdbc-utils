...
public final class SqlLoaderCache {
  ...
  private static Optional<String> findSqlResource(String name, DriverType driverType, ClassLoader classLoader) {
    try {
      InputStream in = classLoader.getResourceAsStream(name + "-" + driverType.name().toLowerCase() + ".sql");
