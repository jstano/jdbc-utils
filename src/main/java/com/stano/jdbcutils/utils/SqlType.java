...
public enum SqlType {
  ...
  STRING(Types.CHAR, Types.VARCHAR, Types.LONGVARCHAR, Types.NCHAR, Types.NVARCHAR, Types.LONGNVARCHAR),
  BYTE(Types.TINYINT),
  ...
  public static SqlType fromSqlType(int sqlType) {
    return Arrays.stream(values()).filter(it -> it.sqlTypes.contains(sqlType)).findFirst().orElseThrow(IllegalArgumentException::new);
  }
