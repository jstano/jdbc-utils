...
public class DbcpDataSourceFactory implements PooledDataSourceFactory {
  ...
  private static final int REMOVE_ABANDONED_TIMEOUT_SECONDS = (int)Duration.ofMinutes(30).getSeconds();
  ...
  public DataSource createPooledDataSource(DataSourceProperties dataSourceProperties) {
    ...
    DataSourceUrlProperties dataSourceUrlProperties = DataSourceUrlParser.parseDataSourceUrl(dataSourceProperties.getUrl());

    dataSource.setJmxName(String.format("org.apache.commons.dbcp2:name=data_source_%s_%s,type=BasicDataSource",
...
//    TelemetryFactory.initialize();
  ...
  private String getConnectionProperties(DataSourceProperties dataSourceProperties) {
    boolean jdbcEncryptionEnabled = Boolean.parseBoolean(System.getProperty("JDBC_ENCRYPTION_ENABLED"));
    boolean jdbcEncryptionTrustCerts = Boolean.parseBoolean(System.getProperty("JDBC_ENCRYPTION_TRUST_CERTS"));
