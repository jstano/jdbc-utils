...
public abstract class AbstractDriver implements JdbcDriver {
   ...
   public void createDatabase(String databaseUrl, String userName, String password) throws RuntimeSQLException {
      ...
      try {
         ...
         String masterDatabaseUrl = "//" + props.getServer() + "/" + getMasterDatabaseName() + props.getOptions();

         try (Connection connection = openConnection(masterDatabaseUrl, userName, password)) {
            try (Statement statement = connection.createStatement()) {
               if (!databaseExists(statement, databaseToCreate)) {
                  statement.executeUpdate(String.format("create database %s", databaseToCreate));
               }
            }
         }
      }
   ...
   protected abstract boolean databaseExists(Statement statement, String databaseName) throws SQLException;
}
