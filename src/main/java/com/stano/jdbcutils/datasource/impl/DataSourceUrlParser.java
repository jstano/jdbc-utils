  ...
  public static DataSourceUrlProperties parseDataSourceUrl(String url) {
    ...
    if (url.startsWith("jdbc:sqlserver:")) {
      ...
      if (matcher.find()) {
        ...
        return new DataSourceUrlProperties(server, database.replace(";databaseName=", ""), options.isEmpty() ? "" : ";" + options);
      }
    }
    else if (url.startsWith("mem:") || url.startsWith("jdbc:h2:mem:") || url.startsWith("jdbc:hsqldb:mem:")) {
      Matcher matcher = Pattern.compile("(?<server>mem)\\:(?<database>[a-zA-Z_0-9\\-\\.]+)(?<optionSeparator>[;\\?])?(?<options>.*)").matcher(url);
      ...
      if (matcher.find()) {
        ...
        return new DataSourceUrlProperties(server, database, options.isEmpty() ? "" : optionSeparator + options);
      }
    }
    else if (url.startsWith("jdbc:xmla:")) {
      Matcher matcher = Pattern.compile(
        "(?<prefix>jdbc:xmla:)?(?<server>Server=[a-zA-Z_0-9\\-\\.\\:\\/]+)(?<catalog>;Catalog=[a-zA-Z_0-9\\-\\.]+);?(?<options>.*)").matcher(url);

      if (matcher.find()) {
        String server = matcher.group("server").replace("Server=", "").replace("http://", "").replace("/olap/msmdpump.dll", "");
