...
public final class MSSQLDriver extends AbstractDriver {
  ...
  public String getFullUrl(String url) {
    ...
    return DRIVER_PREFIX + "//" + props.getServer() + ";databaseName=" + props.getDatabase() + props.getOptions(); //NON-NLS
  }
  ...
  protected boolean databaseExists(Statement statement, String databaseName) throws SQLException {
    try (ResultSet rs = statement.executeQuery(String.format("select 1 from dbo.sysdatabases where name = '%s'", databaseName))) {
      return rs.next();
    }
  }
}
